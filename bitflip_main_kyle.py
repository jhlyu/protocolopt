...
for step in range(training_params['training_iterations']):
    ...
    grad = opt.grad_calc_WIP(simulation_params, protocol_params)
    ...
    a_grad = training_params['alpha_2'] * work_grad_a + training_params['alpha'] * distance_grad_a + training_params['alpha_1'] * var_grad_a
    b_grad = training_params['alpha_2'] * work_grad_b + training_params['alpha'] * distance_grad_b + training_params['alpha_1'] * var_grad_b

    protocol_params['a_list'].grad = a_grad
    protocol_params['b_list'].grad = b_grad

    distance_loss = bitflip.distance_sq_current(phase_data, p_lists, order=2).mean()
    work_mean = grad.work_array(bitflip.potential_value_array(), bitflip.potential_value_advance_array()).sum(axis=-1).mean()
    var_loss = bitflip.var_abs_current(phase_data, p_lists).mean()
    ...
    abs_var_list.append(var_loss.item())
